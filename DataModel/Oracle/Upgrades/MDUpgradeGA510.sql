...
BEGIN
	...
	dbms_output.put_line( 'Marketing Director Schema Upgrade to V5.0.1');
	dbms_output.put_line( '-------------------------------------------');
	dbms_output.put_line( 'Change report:');
	dbms_output.put_line( '    - column PROC_REMOVED_FLG added to CAMP_CYC_STATUS');
	dbms_output.put_line( '      and old records populated with default 0, or 1 for those entries');
	dbms_output.put_line( '      that have completed, i.e.with STATUS_SETTING_ID = 12');
	dbms_output.put_line( '    - column RUN_CYC_NUMBER added to SPI_AUDIT_HDR table');
	dbms_output.put_line( '	   - correction re: MKS 109536');
	dbms_output.put_line( '>');

	execute_sql ('ALTER TABLE CAMP_CYC_STATUS ADD (PROC_REMOVED_FLG NUMBER(1) DEFAULT 0 NOT NULL)');

	execute_sql ('ALTER TABLE CAMP_CYC_STATUS MODIFY PROC_REMOVED_FLG DEFAULT NULL');

	execute_sql ('ALTER TABLE SPI_AUDIT_HDR ADD (RUN_CYC_NUMBER NUMBER(8))');

        update PVDM_UPGRADE set VERSION_ID = '5.0.1.617.2-';
        COMMIT;
...
EXCEPTION
	...
	WHEN NO_DATA_FOUND THEN
		...
		dbms_output.put_line('>');
		dbms_output.put_line('> The upgrade to V5.0.1 is not applicable.');
		dbms_output.put_line('>');
		dbms_output.put_line('>');
...
BEGIN
	select 1 into var_count from pvdm_upgrade where version_id = '5.0.1.617.2-';

	update camp_cyc_status set proc_removed_flg = 1 where status_setting_id = 12;

	COMMIT;

	update camp_comm_in_hdr x set comm_status_id = 1 where exists (select * from camp_comm_out_hdr
	where camp_id = x.camp_id and det_id = x.par_comm_det_id and comm_status_id = 1);
	...
	COMMIT;
        ...
        dbms_output.put_line( '>' );
        dbms_output.put_line( '> Marketing Director Schema has been upgraded to V5.0.1 Build Level 617.2');
	dbms_output.put_line( '>' );
	dbms_output.put_line( '>' );
...
EXCEPTION
	...
	WHEN OTHERS THEN
		dbms_output.put_line('>');
		dbms_output.put_line('> Procedure (5) error occurred.' );
		dbms_output.put_line('>');
		dbms_output.put_line('>');
		RAISE;
END;
...
BEGIN
	...
	dbms_output.put_line( 'Marketing Director Schema Upgrade to V5.1.0');
	dbms_output.put_line( '-------------------------------------------');
	dbms_output.put_line( 'Change report:');
	dbms_output.put_line( '    - views TREAT_COMM_INFO, CHARAC_COMM_INFO, ELEM_COMM_INFO amended');
	dbms_output.put_line( '	   - correction re: MKS 109908');
	dbms_output.put_line( '>');
   ...
   execute_sql ('create or replace view elem_comm_info (a1, camp_id, camp_code, camp_grp_id, camp_name, manager_id, budget_manager_id,
       ...
       CAMP_COMM_OUT_HDR.CAMP_ID = CAMP_COMM_OUT_DET.CAMP_ID AND CAMP_COMM_OUT_HDR.DET_ID = CAMP_COMM_OUT_DET.DET_ID AND 
       CAMP_COMM_OUT_HDR.CAMP_CYC = CAMP_COMM_OUT_DET.CAMP_CYC AND CAMP_COMM_OUT_HDR.RUN_NUMBER = CAMP_COMM_OUT_DET.RUN_NUMBER AND 
       CAMP_COMM_OUT_HDR.COMM_STATUS_ID = CAMP_COMM_OUT_DET.COMM_STATUS_ID AND CAMP_COMM_OUT_HDR.PAR_COMM_DET_ID = CAMP_COMM_OUT_DET.PAR_COMM_DET_ID AND
       CAMP_COMM_OUT_DET.A1 = CAMP_COMM_IN_DET.A1 (+) AND
       CAMP_COMM_OUT_DET.CAMP_ID = CAMP_COMM_IN_DET.CAMP_ID (+) AND CAMP_COMM_OUT_DET.DET_ID = CAMP_COMM_IN_DET.PAR_COMM_DET_ID (+) AND 
       CAMP_COMM_OUT_DET.CAMP_CYC = CAMP_COMM_IN_DET.CAMP_CYC (+) AND CAMP_COMM_OUT_DET.COMM_STATUS_ID = CAMP_COMM_IN_DET.COMM_STATUS_ID (+) AND
       RES_TYPE.RES_TYPE_ID(+) = CAMP_COMM_IN_DET.RES_TYPE_ID AND CAMP_COMM_IN_HDR.CAMP_ID (+) = CAMP_COMM_IN_DET.CAMP_ID AND 
       CAMP_COMM_IN_HDR.DET_ID (+)= CAMP_COMM_IN_DET.DET_ID AND CAMP_COMM_IN_HDR.CAMP_CYC (+) = CAMP_COMM_IN_DET.CAMP_CYC AND 
	...
	execute_sql ('GRANT SELECT ON ELEM_COMM_INFO TO &v3role');
        ...
        update PVDM_UPGRADE set VERSION_ID = '5.1.0.675';
        COMMIT;
...
EXCEPTION
	...
	WHEN NO_DATA_FOUND THEN
		...
		dbms_output.put_line('>');
		dbms_output.put_line('> The upgrade to V5.1.0 is not applicable.');
		dbms_output.put_line('>');
		dbms_output.put_line('>');
