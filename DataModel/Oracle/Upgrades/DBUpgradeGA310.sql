prompt
prompt
prompt 'V3.10 Prime@Vantage Schema Upgrade'
prompt '=================================='
prompt
...
BEGIN
	...
	if (var_version < 107) then
		...
		dbms_output.put_line('> Prime Vantage schema version is below the minimum required release level (BL107)');
		dbms_output.put_line('>');	
	else
		...
		dbms_output.put_line('>');	
		dbms_output.put_line('> The Prime@Vantage Schema has been upgraded to GA 3.0.3, now completing upgrade to V3.1.0.');
		dbms_output.put_line(' ');
	end if;
END;
/
...
prompt 'GA301 Upgrade has been completed.'
